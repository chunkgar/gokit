...
func (r *RedisCluster) XGroupCreate(stream, group string) error {
	if err := r.up(); err != nil {
		return err
	}

	log.Infof("Creating group %s for stream %s", group, stream)
	...
	err := client.XGroupCreateMkStream(stream, group, "0").Err()
	if err != nil {
		// 忽略"BUSYGROUP Consumer Group name already exists"错误
		if strings.Contains(err.Error(), "BUSYGROUP") {
			log.Debugf("Consumer group %s already exists for stream %s", group, stream)
			return nil
		}

		// 忽略stream不存在的错误，因为XGroupCreateMkStream会自动创建stream
		if !errors.Is(err, redis.Nil) {
			log.Errorf("Error trying to create group: %s", err.Error())
			return err
		}
	}
...
func (r *RedisCluster) XReadGroup(stream, group, consumer string, count int64, block time.Duration) ([]redis.XMessage, error) {
	if err := r.up(); err != nil {
		return nil, err
	}

	// log.Debugf("Reading from stream %s with group %s and consumer %s", stream, group, consumer)
	...
	if err != nil {
		...
		log.Errorf("Error trying to read from stream: %s", err.Error())
		return nil, err
	}
...
func (r *RedisCluster) XAdd(stream string, values map[string]interface{}) error {
	if err := r.up(); err != nil {
		return err
	}

	// log.Debugf("Adding to stream %s with values %v", stream, values)
	...
	if err != nil {
		log.Errorf("Error trying to add to stream: %s", err.Error())
...
func (r *RedisCluster) XAck(stream, group string, ids ...string) error {
	if err := r.up(); err != nil {
		return err
	}

	// log.Debugf("Acknowledging messages in stream %s for group %s", stream, group)
	...
	_, err := client.XAck(stream, group, ids...).Result()
	if err != nil {
		log.Errorf("Error trying to acknowledge messages: %s", err.Error())
...
// HSet 设置 hash 字段值
func (r *RedisCluster) HSet(keyName, field, value string) error {
	if err := r.up(); err != nil {
		return err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HSET key: %s, field: %s", fixedKey, field)

	err := r.singleton().HSet(fixedKey, field, value).Err()
	if err != nil {
		log.Errorf("Error trying to set hash field: %s", err.Error())
		return err
	}
...
// HMSet 批量设置多个 hash 字段值
func (r *RedisCluster) HMSet(keyName string, fields map[string]interface{}) error {
	if err := r.up(); err != nil {
		return err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HMSET key: %s, fields count: %d", fixedKey, len(fields))

	err := r.singleton().HMSet(fixedKey, fields).Err()
	if err != nil {
		log.Errorf("Error trying to set hash fields: %s", err.Error())
		return err
	}
...
// HGet 获取 hash 字段值
func (r *RedisCluster) HGet(keyName, field string) (string, error) {
	if err := r.up(); err != nil {
		return "", err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HGET key: %s, field: %s", fixedKey, field)

	value, err := r.singleton().HGet(fixedKey, field).Result()
	if err != nil {
		if errors.Is(err, redis.Nil) {
			log.Debugf("Hash field not found: %s", field)
			return "", ErrKeyNotFound
		}
		log.Errorf("Error trying to get hash field: %s", err.Error())
		return "", err
	}
...
// HMGet 批量获取多个 hash 字段值
func (r *RedisCluster) HMGet(keyName string, fields ...string) ([]interface{}, error) {
	if err := r.up(); err != nil {
		return nil, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HMGET key: %s, fields: %v", fixedKey, fields)

	values, err := r.singleton().HMGet(fixedKey, fields...).Result()
	if err != nil {
		log.Errorf("Error trying to get hash fields: %s", err.Error())
		return nil, err
	}
...
func (r *RedisCluster) HGetAll(keyName string) (map[string]string, error) {
	if err := r.up(); err != nil {
		return nil, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HGETALL key: %s", fixedKey)

	values, err := r.singleton().HGetAll(fixedKey).Result()
	if err != nil {
		log.Errorf("Error trying to get all hash fields: %s", err.Error())
		return nil, err
	}
...
func (r *RedisCluster) HExists(keyName, field string) (bool, error) {
	if err := r.up(); err != nil {
		return false, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HEXISTS key: %s, field: %s", fixedKey, field)

	exists, err := r.singleton().HExists(fixedKey, field).Result()
	if err != nil {
		log.Errorf("Error trying to check hash field existence: %s", err.Error())
		return false, err
	}
...
// HDel 删除 hash 字段
func (r *RedisCluster) HDel(keyName string, fields ...string) (int64, error) {
	if err := r.up(); err != nil {
		return 0, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HDEL key: %s, fields: %v", fixedKey, fields)

	count, err := r.singleton().HDel(fixedKey, fields...).Result()
	if err != nil {
		log.Errorf("Error trying to delete hash fields: %s", err.Error())
		return 0, err
	}
...
// HLen 获取 hash 字段数量
func (r *RedisCluster) HLen(keyName string) (int64, error) {
	if err := r.up(); err != nil {
		return 0, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HLEN key: %s", fixedKey)

	length, err := r.singleton().HLen(fixedKey).Result()
	if err != nil {
		log.Errorf("Error trying to get hash length: %s", err.Error())
		return 0, err
	}
...
// HKeys 获取 hash 所有字段名
func (r *RedisCluster) HKeys(keyName string) ([]string, error) {
	if err := r.up(); err != nil {
		return nil, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HKEYS key: %s", fixedKey)

	keys, err := r.singleton().HKeys(fixedKey).Result()
	if err != nil {
		log.Errorf("Error trying to get hash keys: %s", err.Error())
		return nil, err
	}
...
// HVals 获取 hash 所有值
func (r *RedisCluster) HVals(keyName string) ([]string, error) {
	if err := r.up(); err != nil {
		return nil, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HVALS key: %s", fixedKey)

	values, err := r.singleton().HVals(fixedKey).Result()
	if err != nil {
		log.Errorf("Error trying to get hash values: %s", err.Error())
		return nil, err
	}
...
// HIncrBy 对 hash 字段值进行整数增量操作
func (r *RedisCluster) HIncrBy(keyName, field string, increment int64) (int64, error) {
	if err := r.up(); err != nil {
		return 0, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HINCRBY key: %s, field: %s, increment: %d", fixedKey, field, increment)

	result, err := r.singleton().HIncrBy(fixedKey, field, increment).Result()
	if err != nil {
		log.Errorf("Error trying to increment hash field: %s", err.Error())
		return 0, err
	}
...
// HIncrByFloat 对 hash 字段值进行浮点数增量操作
func (r *RedisCluster) HIncrByFloat(keyName, field string, increment float64) (float64, error) {
	if err := r.up(); err != nil {
		return 0, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HINCRBYFLOAT key: %s, field: %s, increment: %f", fixedKey, field, increment)

	result, err := r.singleton().HIncrByFloat(fixedKey, field, increment).Result()
	if err != nil {
		log.Errorf("Error trying to increment hash field by float: %s", err.Error())
		return 0, err
	}
...
// HSetNX 仅当字段不存在时设置 hash 字段值
func (r *RedisCluster) HSetNX(keyName, field, value string) (bool, error) {
	if err := r.up(); err != nil {
		return false, err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HSETNX key: %s, field: %s", fixedKey, field)

	result, err := r.singleton().HSetNX(fixedKey, field, value).Result()
	if err != nil {
		log.Errorf("Error trying to set hash field if not exists: %s", err.Error())
		return false, err
	}
...
// HSetWithSmartExpire 设置hash字段，如果key不存在则设置过期时间，如果已存在则不重复设置
func (r *RedisCluster) HSetWithSmartExpire(keyName, field, value string, expiration time.Duration) error {
	if err := r.up(); err != nil {
		return err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HSET with smart expire key: %s, field: %s", fixedKey, field)

	client := r.singleton()

	// 检查key是否存在
	exists, err := client.Exists(fixedKey).Result()
	if err != nil {
		log.Errorf("Error checking key existence: %s", err.Error())
		return err
	}

	if exists == 0 {
		// key不存在，使用Pipeline设置值和过期时间
		pipe := client.Pipeline()
		pipe.HSet(fixedKey, field, value)
		pipe.Expire(fixedKey, expiration)

		_, err = pipe.Exec()
		if err != nil {
			log.Errorf("Error trying to set hash field with expiration: %s", err.Error())
			return err
		}
	} else {
		// key已存在，只设置字段值
		err = client.HSet(fixedKey, field, value).Err()
		if err != nil {
			log.Errorf("Error trying to set hash field: %s", err.Error())
			return err
		}
...
// HMSetWithSmartExpire 批量设置hash字段，智能处理过期时间
func (r *RedisCluster) HMSetWithSmartExpire(keyName string, fields map[string]interface{}, expiration time.Duration) error {
	if err := r.up(); err != nil {
		return err
	}

	fixedKey := r.fixKey(keyName)
	log.Debugf("HMSET with smart expire key: %s, fields count: %d", fixedKey, len(fields))
	...
	if exists == 0 {
		// key不存在，使用Pipeline设置值和过期时间
		pipe := client.Pipeline()
		pipe.HMSet(fixedKey, fields)
		pipe.Expire(fixedKey, expiration)

		_, err = pipe.Exec()
		if err != nil {
			log.Errorf("Error trying to set hash fields with expiration: %s", err.Error())
			return err
		}
	} else {
		// key已存在，只设置字段值
		err = client.HMSet(fixedKey, fields).Err()
		if err != nil {
			log.Errorf("Error trying to set hash fields: %s", err.Error())
			return err
		}
